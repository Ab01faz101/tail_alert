...
function getAlertContainer() {
    ...
    if (!container) {
        ...
        container.className =
            "fixed z-50 transition-all flex flex-col items-start justify-start top-0 right-0 h-auto max-h-screen overflow-x-hidden overflow-y-auto w-full sm:max-w-[500px] p-2 sm:p-4";
        document.body.appendChild(container);

        // اضافه کردن استایل برای اسکرول بار سفارشی
        const style = document.createElement("style");
        style.textContent = `
            #alert_boxes_tail_alert::-webkit-scrollbar {
                width: 6px;
            }
            #alert_boxes_tail_alert::-webkit-scrollbar-track {
                background: transparent;
            }
            #alert_boxes_tail_alert::-webkit-scrollbar-thumb {
                background: #d1d5db;
                border-radius: 3px;
            }
            #alert_boxes_tail_alert::-webkit-scrollbar-thumb:hover {
                background: #9ca3af;
            }
        `;
...
function createAlertElement(type, message, description) {
    ...
    <div class="flex flex-col items-start justify-center w-full">
      <h3 class="text-lg font-semibold text-gray-900 break-words flex w-full flex-row justify-between items-center">
        <span>${message}</span>
      ...
      ${description ? `<div class="max-h-40 overflow-y-auto pr-2 w-full"><p class="text-gray-700 text-justify break-words whitespace-normal text-sm">${description}</p></div>` : ""}
    </div>
  ...
  `;
    ...
    alertElem.className = `
    w-full min-h-max relative overflow-hidden transition-all tail_alert_item mb-4 p-4 border border-white rounded-lg flex flex-row items-start space-x-3 shadow-md max-w-full
