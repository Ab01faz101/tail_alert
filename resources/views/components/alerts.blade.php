@if (session('alert'))
    ...
    <section x-data="{ show: true }">
        <section x-show="show" x-cloak x-transition.opacity.scale.75
            ...
            <section
                class="w-[90%] md:w-[40%] bg-white rounded-2xl shadow-2xl overflow-hidden transform transition-all scale-100">
                <!-- Header -->
                <header class="flex items-center justify-between px-6 py-4"
                    :class="{
                        'bg-gradient-to-r from-green-500 to-green-600 text-white': '{{ $type }}'
                        === 'success',
                        'bg-gradient-to-r from-red-500 to-red-600 text-white': '{{ $type }}'
                        === 'error',
                        'bg-gradient-to-r from-yellow-500 to-yellow-600 text-white': '{{ $type }}'
                        === 'warning',
                        'bg-gradient-to-r from-blue-500 to-blue-600 text-white': '{{ $type }}'
                        === 'info'
                    }">
                ...
                <div class="flex flex-col items-center p-6 text-center space-y-4">
                    ...
                    <div class="w-24 h-24 flex items-center justify-center rounded-full shadow-inner"
                        :class="{
                            'bg-green-100 text-green-600': '{{ $type }}'
                            === 'success',
                            'bg-red-100 text-red-600': '{{ $type }}'
                            === 'error',
                            'bg-yellow-100 text-yellow-600': '{{ $type }}'
                            === 'warning',
                        ...
                        <svg class="w-20 h-20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <animate attributeName="stroke-dashoffset" from="22" to="0" dur="0.5s"
                    ...
                    <p class="text-gray-600">{{ __('alert.default_message') }}</p>
                </div>
                ...
                <footer class="flex justify-center p-4 bg-gray-100 border-t">
                    <button @click="show = false"
                        ...
                        :class="{
                            ...
                            === 'success',
                            'bg-red-500 hover:bg-red-600': '{{ $type }}'
                            === 'error',
                            'bg-yellow-500 hover:bg-yellow-600': '{{ $type }}'
                            === 'warning',
